...
const ChapterCard = ({ chapter }) => {
  // Lazy-resolve CQ/MCQ counts on intersection so heavy JSONs only load for
  // chapters that actually become visible.
  ...
  // Pre-warm the chapter route chunk + JSON loaders on hover/touch start so
  ...
  const prefetch = () => {
    ...
    if (typeof window !== 'undefined' && window.__prefetchChapter) {
      window.__prefetchChapter(chapter.id);
    }
  };
