...
import { BookOpen, Sparkles, FlaskConical, ChevronRight, CheckCircle, XCircle, GraduationCap, PenTool, Brain, Target, ShieldCheck, Heart, Stethoscope, Compass } from 'lucide-react';
import { useParams, Link } from 'react-router-dom';

import domainData from './DomainKnowledge.json';
...
export default function DomainKnowledgeDetailsPage() {
  ...
  return (
    <>
      <Helmet>
        <title>{domainData.word_bn} ({domainData.word_en}) - ML by Ramim</title>
        <meta name="description" content={domainData.real_world_flash?.title || "Understand Domain Knowledge in AI and Machine Learning."} />
        <meta name="keywords" content={`Machine Learning, AI, Domain Knowledge, ${domainData.word_en}, ${domainData.word_bn}`} />
        <meta property="og:title" content={`${domainData.word_bn} - Domain Knowledge Simulation`} />
        <meta property="og:description" content="Interactive lab demonstrating why pure math algorithms fail without domain expertise." />
      </Helmet>
      ...
      <div className="flex flex-col gap-3 pb-3 border-b sm:flex-row sm:items-center sm:justify-between border-white/10 md:pb-2">
        ...
        <div className="hidden sm:block text-[10px] md:text-xs font-mono tracking-widest text-slate-500 uppercase">ML Word by Word • Word 24</div>
      </div>
          ...
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed">
            ...
            <motion.div variants={itemVariants} className="pb-3 space-y-1 font-sans border-b md:pb-4 md:space-y-2 border-white/5">
              <div className="flex flex-wrap items-center gap-1.5 md:gap-2 text-[10px] md:text-xs font-bold text-slate-500 tracking-wide uppercase">
                <span className="hidden sm:inline">{domainData.chapter} / {domainData.part}</span>
              </div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl font-extrabold text-slate-100 sm:text-3xl">
                <GraduationCap className="w-6 h-6 text-[#00daf3] md:w-7 md:h-7 shrink-0" />
                {domainData.word_bn}
                <span className="font-sans text-sm font-normal sm:text-lg text-slate-500">({domainData.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="space-y-4 lg:col-span-7">
                <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-wider"><Sparkles size={16} className="text-[#00daf3]"/> {domainData.real_world_flash.title}</div>
                {domainData.real_world_flash.paragraphs.map((p, i) => <p key={i} className="text-justify indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 lg:col-span-5">
                <div className="relative w-full max-w-[240px] md:max-w-[260px] h-64 md:h-72 rounded-2xl border border-white/10 bg-[#0b111b] flex flex-col items-center justify-center p-4 shadow-md overflow-hidden font-sans mx-auto">
                  ...
                  <div className="relative flex items-center justify-center border border-dashed rounded-full w-28 h-28 md:w-32 md:h-32 border-[#00daf3]/30">
                    <Stethoscope size={48} className="w-10 h-10 text-[#00daf3] opacity-70 md:w-12 md:h-12 animate-pulse" />
                  </div>
                  <span className="mt-4 text-[10px] md:text-xs font-mono tracking-widest text-slate-400 font-bold text-center">
                    MEDICAL AI
                  </span>
                  <button onClick={() => { setActiveTab('lab'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="mt-3 px-3 py-1.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-semibold text-slate-500 hover:text-[#00daf3] transition-colors active:scale-95 flex items-center justify-center gap-2">
                    ল্যাবরেটরিতে যাচাই করুন
                  </button>
                </div>
              </div>
            </motion.div>

            <StoryDialogue title={domainData.story_prose.title} dialogues={domainData.story_prose.paragraphs.map(p => ({text: p}))} itemVariants={itemVariants} />

            <LogbookContainer title={domainData.word_bn} subtitle={domainData.word_en} date={domainData.engineering_logbook.date} itemVariants={itemVariants}>
               {domainData.engineering_logbook.points.map((pt, i) => (
                 <LogbookItem key={i} number={pt.id} title={pt.title} icon={i === 3 ? Heart : i === 1 ? Brain : i === 0 ? PenTool : ShieldCheck} isHighlight={i === 3}>
                    {i === 1 ? (
                      <ul className="space-y-2 mt-2">
                         {pt.description.split('\n').map((listItem, idx) => (
                           <li key={idx} className="flex items-start gap-2 text-sm md:text-base text-slate-300">
            ...
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] shadow-md space-y-4 font-sans">
              <h3 className="flex items-start gap-2 text-base font-bold text-slate-100 md:text-lg">
                ...
                <span className="leading-snug">{domainData.readers_reflection.title}</span>
              </h3>
              <p className="font-serif text-sm leading-relaxed text-justify md:text-base text-slate-300 whitespace-pre-line">
                {domainData.readers_reflection.question}
              </p>
              
              <div className="grid grid-cols-1 gap-3 pt-2 md:gap-4 sm:grid-cols-2">
                {domainData.readers_reflection.options.map((opt, i) => (
              ...
              <AnimatePresence>
                {pollSelected && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-3 md:p-4 rounded-lg border bg-white/[0.03] border-white/10 text-slate-300 mt-4 text-xs md:text-sm overflow-hidden">
                    {domainData.readers_reflection.options.map(o => o.id === pollSelected && (
            ...
            <motion.div variants={itemVariants} className="pt-4">
              <WordNavigation fallbackPath="domain-knowledge" />
            </motion.div>

          </motion.div>
        ) : (
          <motion.div key="lab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="pt-2 pb-10 md:pt-4">
            <Suspense fallback={<div className="flex flex-col items-center justify-center h-64 text-slate-400 gap-3"><Compass className="animate-spin text-[#00daf3]" size={32}/><span className="text-sm font-bold tracking-widest uppercase">ল্যাবরেটরি প্রস্তুত হচ্ছে...</span></div>}>
              <SimulationLab />
            </Suspense>
          </motion.div>
        )}
