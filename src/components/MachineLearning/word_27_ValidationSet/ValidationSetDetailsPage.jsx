...
import { Target, Sparkles, BookOpen, FlaskConical, Layers, Heart, Compass, CheckCircle, XCircle, ChevronRight, Activity, ShieldCheck, Gauge } from 'lucide-react';
import { useParams, Link } from 'react-router-dom';
import { Helmet } from 'react-helmet-async';

import valData from './ValidationSet.json';
import SEO from '../../SEO';
...
export default function ValidationSetDetailsPage() {
  ...
  const seoSchema = {
    ...
    "@type": "TechArticle",
    "headline": `${valData.word_bn} (${valData.word_en}) - Machine Learning Book`,
    "description": "ভ্যালিডেশন সেট কী? মেশিন লার্নিংয়ে মডেলের ভুলত্রুটি ধরা এবং হাইপারপ্যারামিটার টিউনিং করার জন্য ভ্যালিডেশন সেটের গুরুত্ব সম্পর্কে জানুন।",
    "author": {
      "@type": "Person",
      "name": "Ramim"
    },
    "keywords": "Validation Set, ভ্যালিডেশন সেট, Machine Learning, Hyperparameter Tuning, Model Testing, ML in Bengali"
  };
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 py-4 md:py-8 space-y-6 md:space-y-8 text-slate-300 bg-[#070b12]">
      <SEO 
        title={`${valData.word_bn} (${valData.word_en})`}
        description="ভ্যালিডেশন সেট কী? মেশিন লার্নিংয়ে মডেলের ভুলত্রুটি ধরা এবং হাইপারপ্যারামিটার টিউনিং করার জন্য ভ্যালিডেশন সেটের গুরুত্ব সম্পর্কে জানুন।"
        schema={seoSchema}
      />
      <Helmet>
        <meta name="keywords" content="Validation Set, ভ্যালিডেশন সেট, Machine Learning, Hyperparameter Tuning, Model Testing, ML in Bengali" />
      </Helmet>
      ...
      <div className="flex flex-col gap-3 pb-3 border-b sm:flex-row sm:items-center sm:justify-between border-white/10 md:pb-2">
        <div className="flex w-full gap-2 sm:gap-4 sm:w-auto">
          <button onClick={() => setActiveTab('reading')} className={`flex-1 sm:flex-none relative flex items-center justify-center sm:justify-start gap-2 pb-2 font-bold text-xs sm:text-sm transition-colors ${activeTab === 'reading' ? 'text-slate-100' : 'text-slate-500 hover:text-slate-300'}`}>
            ...
            {activeTab === 'reading' && <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-0.5 bg-amber-400" />}
          </button>
        ...
        <div className="hidden sm:block text-[10px] md:text-xs font-mono tracking-widest text-slate-500 uppercase">ML Word by Word • Word 27</div>
      </div>
        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed">
            ...
            <motion.div variants={itemVariants} className="pb-3 space-y-1 border-b md:pb-4 border-white/5">
              <div className="text-[10px] md:text-xs font-bold text-slate-500 uppercase">{valData.chapter} / {valData.part}</div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl font-extrabold text-slate-100 sm:text-3xl lg:text-4xl">
                <Gauge className="w-6 h-6 text-amber-500 md:w-8 md:h-8" /> {valData.word_bn} <span className="text-sm font-normal text-slate-500 sm:text-lg">({valData.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="space-y-4 lg:col-span-7">
                <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-wider"><Sparkles size={16} className="text-amber-500"/> {valData.real_world_flash.title}</div>
                {valData.real_world_flash.paragraphs.map((p, i) => <p key={i} className="text-justify indent-6">{p}</p>)}
              </div>
              <div className="lg:col-span-5 bg-[#0b111b] border border-white/5 rounded-3xl p-8 text-center relative overflow-hidden shadow-2xl">
                 ...
                 <div className="text-6xl mb-4 text-amber-500 animate-pulse">🛠️</div>
                 <p className="text-sm font-bold text-slate-200 mb-4 tracking-widest uppercase">Tuning Phase</p>
                 <div className="bg-white/[0.02] p-4 rounded-xl border border-white/5 font-mono text-[10px] text-amber-400 text-left space-y-1 shadow-inner">
                    <p>Load: 15% Validation Data</p>
                    <p>Action: Tweaking Hyperparameters</p>
                    <p>Status: Iterative Improvements</p>
                 </div>
                 <button onClick={() => { setActiveTab('lab'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="mt-6 w-full py-3 bg-amber-600/20 hover:bg-amber-600 text-amber-400 hover:text-slate-900 border border-amber-500/30 rounded-xl text-xs font-bold transition-all active:scale-95">গাড়ি টিউনিং ল্যাবে যান</button>
              </div>
            </motion.div>

            <StoryDialogue title={valData.story_prose.title} dialogues={valData.story_prose.paragraphs.map(p => ({text: p}))} itemVariants={itemVariants} />
            <ComparisonTable tableData={valData.comparison_table} itemVariants={itemVariants} />

            <LogbookContainer title={valData.word_bn} subtitle={valData.word_en} date={valData.engineering_logbook.date} itemVariants={itemVariants}>
               {valData.engineering_logbook.points.map((pt, i) => (
                 <LogbookItem key={i} number={pt.id} title={pt.title} icon={i === 3 ? Heart : i === 2 ? Layers : i === 1 ? Target : ShieldCheck} isHighlight={i === 3}>
              ...
              <AnimatePresence>
                {pollSelected && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-3 md:p-4 rounded-lg border bg-white/[0.03] border-white/10 text-slate-300 mt-4 text-xs md:text-sm overflow-hidden">
                    {valData.readers_reflection.options.map(o => o.id === pollSelected && (
            ...
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] font-sans">
              <div className="space-y-3">
                <span className="text-[9px] md:text-[10px] font-mono font-bold tracking-widest text-slate-400 uppercase flex items-center gap-1">
                  <Compass size={12} /> ENTERING CROSS VALIDATION
                </span>
                <h3 className="text-base font-bold text-slate-100 md:text-lg">পরবর্তী ধাপে যেতে প্রস্তুত?</h3>
                <p className="text-xs leading-relaxed md:text-sm text-slate-300">{valData.next_intro.text}</p>
                <div className="pt-2">
                  <Link to={`/book/${bookSlug}/foundation/28`} className="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-5 py-2.5 rounded-lg bg-[#1f3a46] text-slate-100 font-bold text-sm hover:bg-[#294957] transition-all group active:scale-95">
                    পরবর্তী শব্দে যান (Cross Validation) <ChevronRight size={14} className="transition-transform group-hover:translate-x-1" />
                  </Link>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="pt-4">
              <WordNavigation fallbackPath="validation-set" />
            </motion.div>
