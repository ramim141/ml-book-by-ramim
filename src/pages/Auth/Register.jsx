...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    if (password !== confirmPassword) {
      return setError('পাসওয়ার্ড ম্যাচ করেনি!');
    }
    ...
    try {
      ...
      console.error(err);
      setError('অ্যাকাউন্ট তৈরি করা যায়নি। ইমেইলটি আগে থেকেই ব্যবহৃত হতে পারে।');
    }
    setLoading(false);
  }
  ...
  return (
    <div className="min-h-screen bg-[#050914] flex items-center justify-center py-6 sm:py-12 px-4 sm:px-6 lg:px-8 font-bangla relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-fuchsia-600/10 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="max-w-md w-full space-y-5 sm:space-y-8 bg-slate-900/50 p-6 sm:p-8 rounded-3xl border border-slate-800 backdrop-blur-xl relative z-10 shadow-2xl">
        <div>
          <div className="w-12 h-12 sm:w-16 sm:h-16 bg-fuchsia-500/20 rounded-2xl border border-fuchsia-500/30 flex items-center justify-center mx-auto mb-4 sm:mb-6">
            <UserPlus className="w-6 h-6 sm:w-8 sm:h-8 text-fuchsia-400" />
          </div>
          ...
          <p className="mt-2 text-center text-xs sm:text-sm text-slate-400 font-medium">
            অ্যাডমিন প্যানেলের অ্যাক্সেস পেতে সাইন আপ করুন
          </p>
        </div>
        ...
        <form className="mt-6 sm:mt-8 space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                autoComplete="email"
                required
                className="w-full pl-11 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-fuchsia-500/50 focus:ring-4 focus:ring-fuchsia-500/10 transition-all placeholder:text-slate-500"
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-fuchsia-600 hover:bg-fuchsia-700 focus:outline-none focus:ring-4 focus:ring-fuchsia-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-fuchsia-600/20"
            >
              {loading ? (
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                console.error(err);
                setError('Google দিয়ে সাইন আপ করা যায়নি।');
                setLoading(false);
              }
            }}
