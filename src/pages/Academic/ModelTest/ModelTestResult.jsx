...
export default function ModelTestResult() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] pt-24 pb-24 px-4 sm:px-6 lg:px-8 font-bangla selection:bg-fuchsia-500/30">
      <div className="max-w-4xl mx-auto space-y-8">
        
        {/* Score Card */}
        <div className="bg-slate-800/40 border border-slate-700/50 rounded-3xl p-6 sm:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
            ...
            <div className="relative w-40 h-40 shrink-0 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-bold text-white">{enToBnNumber(percentage)}%</span>
                <span className="text-xs text-slate-400 font-medium mt-1">সঠিক</span>
              </div>
            </div>

            {/* Stats */}
            <div className="flex-1 w-full">
              <h2 className="text-2xl sm:text-3xl font-black text-white mb-2 text-center md:text-left">পরীক্ষার ফলাফল</h2>
              <p className="text-slate-400 font-medium mb-6 text-center md:text-left">{subjectTitle}</p>
              
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-700/50 text-center">
                  <div className="text-2xl font-bold text-white">{enToBnNumber(questions.length)}</div>
                  <div className="text-xs font-medium text-slate-400 mt-1">মোট প্রশ্ন</div>
                </div>
                <div className="bg-emerald-500/10 p-4 rounded-2xl border border-emerald-500/20 text-center">
                  <div className="text-2xl font-bold text-emerald-400">{enToBnNumber(correctCount)}</div>
                  <div className="text-xs font-medium text-emerald-500 mt-1">সঠিক উত্তর</div>
                </div>
                <div className="bg-rose-500/10 p-4 rounded-2xl border border-rose-500/20 text-center">
                  <div className="text-2xl font-bold text-rose-400">{enToBnNumber(wrongCount)}</div>
                  <div className="text-xs font-medium text-rose-500 mt-1">ভুল উত্তর</div>
                </div>
                <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-700/50 text-center">
                  <div className="text-2xl font-bold text-slate-300">{enToBnNumber(skippedCount)}</div>
                  <div className="text-xs font-medium text-slate-400 mt-1">বাদ দেওয়া</div>
                </div>
              </div>
            </div>
          </div>
          
          <div className="mt-8 flex flex-col sm:flex-row items-center gap-4 pt-6 border-t border-slate-700/50">
            <div className="flex items-center gap-2 text-slate-400 font-medium bg-slate-900/50 px-4 py-2 rounded-xl">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white transition-colors w-full sm:w-auto justify-center"
            >
              <RotateCcw className="w-4 h-4" /> পুনরায় পরীক্ষা দিন
            </button>
            <button
              onClick={() => navigate('/academic')}
              className="px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white transition-colors w-full sm:w-auto justify-center"
            >
            ...
            {questions.map((q, idx) => {
              ...
              return (
                <div key={idx} className={`p-5 sm:p-6 rounded-2xl border ${statusBorder} ${statusBg} transition-all`}>
                  ...
                  {q.explanation && (
                    <div className="ml-12 mt-4 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-xl">
